...
constexpr bool is_in(uint32_t ptr, range& r) { return ptr >= r.begin && ptr < r.end; }
...
static bool is_page_reserved(uint64_t page) {
   for(int i = 0; i < num_reserved_ranges; i++) {
      if(is_in(page * arch::page_size, reserved_memory[i]) ||
...
static paddr_t early_pmm_alloc_cts(uint64_t pages, int slimit = 10) {
   ...
   paddr_t result;
   if(search_run(pages, page_bas, page_lim, result)) return result;
   ++mmap_it;
...
void bootstrap_pmm(const range (&res)[1], struct multiboot_tag_mmap* mmap) {
   ...
   ebl::klog("PFN DB size: %lu free + %lu used = %lu total\n",
             (uint64_t)pfndb_freelist.size(),
             (uint64_t)pfndb_rsrvlist.size(),
             (uint64_t)pfndb_freelist.size() + pfndb_rsrvlist.size());
   ebl::klog("PFN DB addr: 0x%lx - 0x%lx\n",
             (paddr_t)pfndb_arr,
             (paddr_t)pfndb_arr + pfndb_sz_bytes - 1);
...
uint64_t setup_paging() {
   ...
   pml4[511].data =
         (uint64_t)pml4 | ns::page_flags{.f{.present = 1, .writable = 1, .user = 0}}.data;
