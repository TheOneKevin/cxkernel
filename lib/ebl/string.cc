...
namespace ebl {
   ...
   int memcmp(const void* buf1, const void* buf2, size_t len) {
      const unsigned char* p1 = static_cast<const unsigned char*>(buf1);
      const unsigned char* p2 = static_cast<const unsigned char*>(buf2);
      while(len--)
         if(*p1 != *p2)
            return *p1 - *p2;
         else
...
#if ARCH_TYPE == ARCH_x86_32
      asm volatile("rep stosb" : : "a"(c), "D"(dest), "c"(len) : "memory");
      return dest;
#else
   ...
   size_t strlen(const char* str) {
      ...
      for(ptr = str; ((unsigned long int)ptr & (sizeof(longword) - 1)) != 0; ++ptr)
...
extern "C" void* memset(void* dest, int c, size_t len) { return ebl::memset(dest, c, len); }
