...
static int parse_num(const char** c) {
   int num = 0;
   while(is_digit(**c)) num = num * 10 + *((*c)++) - '0';
   return num;
}
...
static int num(void (*g)(char, char**),
               char** _buf,
               long long num,
               char base,
               uint8_t flags,
               ...
               int percision) {
   char str[23];
   memset(str, 0, 23); // 22 digits of uint64_t in oct + 1 EOL
   ...
   if(((flags & SIGNED) && (num < 0)) || ((flags & SIGNED) && (flags & FPLUS)) ||
...
static int ssprintf(void (*g)(char, char**), char* buf, const char* fmt, va_list args) {
   int written = 0;
   uint8_t flags;
   int width;
   int percision;
         ...
         switch(*fmt) {
            ...
            case 'u':
               ...
               if(length == 'l')
                  ...
                  val = (unsigned short)va_arg(args, uint32_t);
               else
            ...
            case 'p':
               ...
               written += num(
                     g, &buf, (unsigned long long)va_arg(args, void*), 16, flags, width, percision);
               break;
            case 'i':
            case 'd':
               flags |= SIGNED;
...
static char* stream_buffer[] = {&stream_buffer_org[0][0], &stream_buffer_org[1][0]};
...
static void stream_log(char c, char** buf) {
   (void)buf;
   *stream_buffer[1]++ = c;
   if(c == '\n' || (stream_buffer[1] - &stream_buffer_org[1][0]) > 512) fflush(ebl::CLOG);
}

static void stream_out(char c, char** buf) {
   (void)buf;
   *stream_buffer[0]++ = c;
   if(c == '\n' || (stream_buffer[0] - &stream_buffer_org[0][0]) > 512) fflush(ebl::COUT);
}
