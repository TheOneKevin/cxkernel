    ...
    public:
        ...
        template<typename T> header<T> get_header() {
            static_assert(ebl::always_false<T>::value,
                "Unsupported type <T> for get_header()");
        }
        template<> header<elf64_phdr_t> get_header<elf64_phdr_t>() {
            return header<elf64_phdr_t>{impl64_.phdr, impl64_.img->e_phnum};
        }
        // FIXME: Should this be genericized too?
        void* get_image() const {
            return is_64bits_ ? (void*) impl64_.img : (void*) impl32_.img;
        }
    ...
    private:
        ...
        private:
            struct iterator {
                ...
                iterator& operator++() { hdr_++; return *this; }
