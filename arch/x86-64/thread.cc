...
namespace arch {
   ...
   void switch_thread(core::Thread* oldthread, core::Thread* newthread) {
      ...
      asm volatile("pushf" NL      // Push flags
                   "push %%rbx" NL // Push callee-save regs
                   "push %%rbp" NL "push %%r12" NL "push %%r13" NL "push %%r14" NL "push %%r15" NL
                   "mov %%rsp, (%0)" NL // Save old rsp
                   "mov %1, %%rsp" NL   // Load new rsp
                   "pop %%r15" NL       // Restore callee-save regs
                   "pop %%r14" NL "pop %%r13" NL "pop %%r12" NL "pop %%rbp" NL "pop %%rbx" NL
                   "popf" NL // Restore flags
                   ::"r"(&oldthread->backend.sp),
                   "r"(&newthread->backend.sp));
   }
