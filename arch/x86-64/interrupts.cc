...
static x86_64::idt_ptr idt_reg_ptr{.limit = sizeof(idt) - 1, .base = (uint64_t)&idt};
...
void arch::irq_remove_handler(IrqVector vector) { irq_handlers[vector.vector] = nullptr; }

arch::irq_handler_t arch::irq_get_handler(IrqVector vector) { return irq_handlers[vector.vector]; }

static const char* exception_messages[] = {"#DE Division By Zero",
                                           "#DB Debug",
...
void x86_64::init_idt() {
   for(int i = 0; i < 256; i++) {
      idt[i] = x86_64::idt_entry{(&isr_stub_table)[i], x86_64::descriptor_type::INTERRUPT_GATE};
