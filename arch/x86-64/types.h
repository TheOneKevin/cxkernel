...
namespace x86_64 {
   ...
   constexpr uint64_t virt_from_index(uint16_t pml4e_index,
                                      uint16_t pdpte_index,
                                      uint16_t pde_index,
                                      uint16_t pte_index) {
      return canonicalize(
            ((uint64_t)(pml4e_index & 0x1FF) << 39) | ((uint64_t)(pdpte_index & 0x1FF) << 30) |
            ((uint64_t)(pde_index & 0x1FF) << 21) | ((uint64_t)(pte_index & 0x1FF) << 12));
   }

   // Gets the index of the PML4 entry for a virtual address
   // ref: Figure 4-8 from Intel SDM Vol 3 4.5.4
   constexpr uint16_t pml4e_index(vaddr_t addr) { return (addr >> 39) & 0x1FF; }
   // ref: pml4e_index
   constexpr uint16_t pdpte_index(vaddr_t addr) { return (addr >> 30) & 0x1FF; }
   // ref: pml4e_index
   constexpr uint16_t pde_index(vaddr_t addr) { return (addr >> 21) & 0x1FF; }
   // ref: pml4e_index
   constexpr uint16_t pte_index(vaddr_t addr) { return (addr >> 12) & 0x1FF; }
} // namespace x86_64
