...
static slabcache slabcache_cache_node{
      "slabcache cache", align_up(sizeof(slabcache), default_order), default_order};
...
slabcache* find_best_cache(unsigned int size) REQUIRES(cache_list_lock) {
   for(auto cache : cache_list) {
      ...
      if(cache->slab_objsize == size) return cache;
   }
   return nullptr;
}

slab* find_slab_from_obj(void* obj) { return (slab*)arch::page_align_down((vaddr_t)obj); }
...
slab* slab_create(slabcache* cache) REQUIRES(cache->lock) {
   ...
   node = new(node) slab{cache, (objctl*)ptr, (void*)ptr};
...
void kmem::free(void* obj) {
   ...
   assert(arch::is_heap_address((vaddr_t)node), "Object is not in the heap");
   slabcache* cache = node->parent;
   assert(align_up((vaddr_t)obj, cache->order) == (vaddr_t)obj,
          "Object is not within the expected alignment");
   assert((vaddr_t)obj >= (vaddr_t)node->objects, "Object is not within the expected slab range");
