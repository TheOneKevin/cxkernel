...
namespace core {
    ...
    enum struct VmRegionType : uint8_t {
        REGION = 0,
        MAPPING = 1
    };
    ...
    private:
        VmRegion(
            vaddr_t base, vaddr_t size,
            VmRegionFlags flags, AddressSpace* aspace
        ) noexcept :
            base_{base}, size_{size}, flags_{flags},
            parent_{nullptr}, aspace_{aspace} { }
    public:
        /**
         * @brief 
         * 
    ...
    private:
         ...
         */
        template<typename T> void foreach_hole(T func) {
            ...
            for(auto child : children_) {
                if(child->base_ > this->base_ + offset) {
                    if(!func(child, offset, child->base_ - this->base_ - offset))
                        return;
                }
