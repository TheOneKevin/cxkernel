/*
 * Filename: kprintf-test.c
 * Author:   Kevin Dai
 * Email:   kevindai02@outlook.com
 *
 * Created on 01-Aug-2017 04:14:22 PM
 *
 * @ Last modified by:   Kevin Dai
 * @ Last modified time: 12-Aug-2017 02:50:18 PM
*/
...
void __test_kprintf__()
{
    ...
    ASSERT(strcmp(buf, test_msg[0]), "printk test case [1] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[2]: %s %x %X %u %o %c %% %d %i \n", "Hello, world!", 0xDEADBEEF, 0xCAFEBABE, 10, 8, 'c', 'd', -10, 0);
    ASSERT(strcmp(buf, test_msg[1]), "printk test case [2] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[3]: %0#8X %#010x %#4o %0*u\n", 0xDEAD, 0xBEEF, 8, 4, 10);
    ASSERT(strcmp(buf, test_msg[2]), "printk test case [3] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[4]: %ld 0x%lX 0x%hX %hi\n", myLong, 0xFEFFFFFFBFFFFFFFL, 0xDEADBEEF, myLong);
    ASSERT(strcmp(buf, test_msg[3]), "printk test case [4] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[5]: %+ 6d %10s %10c\n", -1024, "string", 'c');
    ASSERT(strcmp(buf, test_msg[4]), "printk test case [5] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[6]: % 6d %10i %#010X\n", 1024, -123456, 0xFEEDFACE);
    ASSERT(strcmp(buf, test_msg[5]), "printk test case [6] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[7]: %X %lX\n", (int) 0xDECAFC0FFEEL, 0xDECAFC0FFEEL);
    ASSERT(strcmp(buf, test_msg[6]), "printk test case [7] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[8]: %-15s %15s\n", "poop is fun", "fun is poop");
    ASSERT(strcmp(buf, test_msg[7]), "printk test case [8] failed\n");
    memset((void *) buf, 0, 80);

    sprintf(buf, "[9]: %d %u %X %x %o %+d % d %#X %#x %#o %#u\n\n", -1024, 1024, 0xBEEF, 0xCAFE, 8, 1024, 1024, 0xBEEF, 0xCAFE, 8, 10);
    ASSERT(strcmp(buf, test_msg[8]), "printk test case [9] failed\n");
    memset((void *) buf, 0, 80);
}
