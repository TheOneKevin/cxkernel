...
#define LIBALLOC_DEBUG
 ...
 #include "lib/printk.h"
 #define FLUSH() fflush(STREAM_OUT)
#endif
...
#if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
    static void liballoc_dump(void)
    {
...
void *PREFIX(malloc)(size_t req_size)
{
    ...
    if (l_memRoot == NULL)
    {
        ...
        if (l_memRoot == NULL)
        {
            liballoc_unlock();
            #if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
                fprintf(STREAM_ERR, "liballoc: initial l_memRoot initialization failed\n", p);
                FLUSH();
            #endif
            return NULL;
        }
    ...
    while (maj != NULL)
    {
        ...
        if (bestSize < diff)
        {
            ...
            {
                #ifdef LIBALLOC_DEBUG
                    fprintf(STREAM_LOG, "CASE 1: Insufficient space in block %x\n", maj);
                    FLUSH();
                #endif
            ...
            if (maj -> first == NULL)
            {
                ...
                #ifdef LIBALLOC_DEBUG
                    fprintf(STREAM_LOG, "CASE 2: returning %x\n", p);
                    FLUSH();
                #endif
            ...
            if (diff >= (size + sizeof(struct liballoc_minor)))
            {
                ...
                #ifdef LIBALLOC_DEBUG
                    fprintf(STREAM_LOG, "CASE 3: returning %x\n", p);
                    FLUSH();
                #endif
                liballoc_unlock(); // release the lock
                return p;
            ...
            while (min != NULL)
            {
                ...
                if (min -> next == NULL)
                {
                    ...
                    if (diff >= (size + sizeof(struct liballoc_minor)))
                    {
                        ...
                        #ifdef LIBALLOC_DEBUG
                            fprintf(STREAM_LOG, "CASE 4.1: returning %x\n", p);
                            FLUSH();
                        #endif
                        liballoc_unlock(); // release the lock
                        return p;
                ...
                if (min -> next != NULL)
                {
                    ...
                    if (diff >= (size + sizeof(struct liballoc_minor)))
                    {
                        ...
                        #ifdef LIBALLOC_DEBUG
                            fprintf(STREAM_LOG, "CASE 4.2: returning %x\n", p);
                            FLUSH();
                        #endif
            ...
            {
                #ifdef LIBALLOC_DEBUG
                    fprintf(STREAM_LOG, "CASE 5: block full\n");
                    FLUSH();
                #endif
    ...
    #ifdef LIBALLOC_DEBUG
        fprintf(STREAM_ERR, "All cases exhausted. No memory available.\n");
        FLUSH();
    #endif
    #if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
...
void PREFIX(free)(void *ptr)
{
    ...
    if (min -> magic != LIBALLOC_MAGIC)
    {
            ...
            )
        {
            ...
            #if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
                fprintf(STREAM_ERR, "liballoc: ERROR: Possible 1-3 byte overrun for magic!"); // Don't display MAGIC for security
                FLUSH();
            #endif
        }
        ...
        {
            #if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
                fprintf(STREAM_ERR, "liballoc: ERROR: multiple free() attempt on %x from %x\n", ptr, __builtin_return_address(0));
                FLUSH();
            #endif
        }
        else
        {
            #if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
                fprintf(STREAM_ERR, "liballoc: ERROR: Bad free(%x) called from %x\n", ptr, __builtin_return_address(0));
                FLUSH();
            #endif
        }
...
void* PREFIX(realloc)(void *p, size_t size)
{
    ...
    if (min -> magic != LIBALLOC_MAGIC)
    {
            ...
            ((min -> magic & 0xFF) == (LIBALLOC_MAGIC & 0xFF)))
        {
            l_possibleOverruns += 1;
            #if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
                fprintf(STREAM_ERR, "liballoc: ERROR: Possible 1-3 byte overrun for magic!\n");
                FLUSH();
            #endif
        }
