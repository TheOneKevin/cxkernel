 ...
 * @ Last modified time: 2017-11-26T12:46:15-05:00
*/
...
elf_sheader_t* mb_elf_get_section_header(char* sname)
{
    ...
    uint32_t addr  = multiboot_elf_sec -> addr + ARCH_VIRT_BASE;
    ...
    for(uint32_t i = 0; i < num; i++)
    {
        ...
        char* name = (char *) (shstr -> sh_addr + ARCH_VIRT_BASE + shdr -> sh_name);
...
void mb_elf_print_sections(void)
{
    ...
    {
        elf_sheader_t* shdr = (elf_sheader_t *) (addr + i * sizeof(elf_sheader_t));
        if(((char *) (shstr -> sh_addr + ARCH_VIRT_BASE + shdr -> sh_name))[0] == 0)
            fprintf(STREAM_LOG, "[.null]\n");
        else
            fprintf(STREAM_LOG, "[%s]\n", (void *) (shstr -> sh_addr + ARCH_VIRT_BASE + shdr -> sh_name));
    }
}
